              ...
              return (
                <div key={idx} className="column_style">
                  ...
                  <div className="flex flex-col text-sm font-semibold my-7">
                    {x.urls.map((url, i) => {
                      return (
                        <Link href={url.url} target={url.target} key={url.title} className="hover:text-bm-blue my-2">
                          {url.title}
                        </Link>
                      )
                    })}
            ...
            <div className="contact_section relativeStyle">
              ...
              <div className=" social_icons_div flex justify-between w-[450px] pb-5">
                {socialUrls.map((x, idx) => {
                  return (
                    <div key={idx} className=" h-11 w-11 border-solid border-[#3174D0] border-2 rounded-full ">
                      <SocialIcon
                        url={x}
                        target="_blank"
                        bgColor="transparent"
